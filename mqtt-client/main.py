...
class MQTTClient(OMPluginBase):
    ...
    def _load_input_configuration(self):
        ...
        if self._input_enabled:
            try:
                ...
                self.logger('Error while loading input configurations')
                input_config_loaded = False
            try:
                ...
                self.logger('Error getting input status')
                input_config_loaded = False
    ...
    def _load_output_configuration(self):
        ...
        if self._output_enabled:
            try:
                ...
                output_config_loaded = False
                self.logger('Error while loading output configurations')
            try:
                ...
                output_config_loaded = False
                self.logger('Error getting output status')
    ...
    def _load_sensor_configuration(self):
        ...
        if self._sensor_enabled:
            try:
                ...
                sensor_config_loaded = False
                self.logger('Error while loading sensor configurations')
    ...
    def _load_power_configuration(self):
        ...
        if self._power_enabled:
            try:
                ...
                power_config_loaded = False
                self.logger('Error while loading power configurations')
    ...
    def _try_connect(self):
        if self._enabled is True:
            try:
                ...
                self.logger('Error connecting to MQTT broker')
    ...
    def _send(self, topic, data, qos, retain):
        try:
            ...
            self.logger('Error sending data to broker')
    ...
    def input_status(self, data):
        if self._enabled and self._input_enabled:
            ...
            try:
                ...
                self.logger('Error processing input {0}'.format(input_id))
    ...
    def output_status(self, status):
        if self._enabled and self._output_enabled:
            try:
                ...
                self.logger('Error processing outputs')
                ...
                if self._enabled:
                    ...
                    while frequency >= 10:
                        ...
                        try:
                            ...
                            self.logger('Error processing {0} sensor status'.format(sensor_type))
    ...
    def on_connect(self, client, userdata, flags, rc):
        ...
        if self._output_command_topic:
            try:
                ...
                self.logger('Could not subscribe')
    ...
    def set_config(self, config):
        try:
            ...
            self.logger('Error saving configuration')
